...
if (envAddress) {
    try {
        TARGET_ADDRESS = core_1.Address.parse(envAddress).toString({
            urlSafe: true,
            bounceable: false
        });
    } catch (e) {
...
function main() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (args['--wallet'] === 'highload') {
            console.log('Using highload wallet', wallet.address.toString({
                bounceable: false,
                urlSafe: true
            }));
        } else {
            console.log('Using v4r2 wallet', wallet.address.toString({
                bounceable: false,
                urlSafe: true
            }));
        }
        const targetAddress = TARGET_ADDRESS !== null && TARGET_ADDRESS !== void 0 ? TARGET_ADDRESS : wallet.address.toString({
            bounceable: false,
            urlSafe: true
        });
        ...
        while (go) {
            ...
            const mined = yield new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                ...
                for (let i = 0; i < gpus; i++) {
                    ...
                    const procid = (0, child_process_1.spawn)(bin, command.split(' '), {
                        stdio: "pipe"
                    });
                    // procid.on('message', (m) => {
